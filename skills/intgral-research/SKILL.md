...
Every new report save carries `runbook_revision: intgral-research@3` and `schema_revision: <report_kind>/1`; the function reference names its `template_revision` and `skill_revision`. Preserve all revision values on historical reports.
