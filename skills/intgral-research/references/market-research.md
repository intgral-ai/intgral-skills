...
Worked example with positive, incomplete and conflicting variants: [examples/market.md](examples/market.md).
...
3. Separate measured demand from hypotheses inferred from offers, reviews or search interest.
   `measured_metrics` stays null unless a retained `research-market-measurement/1` record is repeated exactly; badge floors summed over the retained listings may appear only as an `inference` claim that names the listings and says "lower bound, not a category figure". A cheaper alternative may sit in `competition_prices` when it shares currency, unit and quantity, labelled as an alternative outside the direct group. Compare each hypothesis against alternative explanations and counterevidence.
...
Save as `market_research` ([payload](report-data/market_research.md), [artifacts and reuse](artifacts-and-reuse.md)) with `template_revision: market_research@1` and `skill_revision: intgral-research/market@2`.
