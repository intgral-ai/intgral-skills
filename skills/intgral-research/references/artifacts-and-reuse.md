...
Read a retained clip and its sampled frames with `medusa.admin_get` on the catalogued routes: `GET /admin/research/video-references?scope_id=<scope>` lists a scope's references, each row carrying `state`, `frames_state`, `frames_failure_reason` and `frames_count` — `frames_count` and `frames_failure_reason` let an agent decide whether to open the detail without minting frame URLs; `GET /admin/research/video-references/:id` reads one reference's detail, adding `media_access.url` for the clip when `state` is `ready`, and `frames[]` with `index`, `timestamp_seconds`, `sha256`, `bytes` and a per-frame `access.url` when `frames_state` is `ready`. Reads never collect and never trigger extraction; a URL in a response is not evidence that anything was watched.
